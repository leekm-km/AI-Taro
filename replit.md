...
- **Character Image System (Oct 8)**: Integrated 33 character images across 5 characters (Lucien 9개, Linhua 8개, Isolde 4개, Cheongun 5개, Thimble 7개) with random selection and duplicate prevention system. All images renamed from Korean to English filenames to resolve NFD encoding issues in Flutter Web. Images display as large 1:1 square format throughout the app: CharacterSelectPage uses AspectRatio containers for full-width 1:1 display, FortuneCategoryPage/QuestionPage/CardSelectionPage use 200x200 square images, and ResultPage displays three 300x300 uniform square images (one above GPT response, two interspersed within reading text). Implemented usedImages tracking across entire navigation flow to prevent image repetition within a single session.
